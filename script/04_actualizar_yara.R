...
read_yara(device_id_sm,from_date = "2023-10-01 00:00",
...
data_sm |> 
  ggplot(aes(hora, value, color = as.factor(codigo))) +
  geom_point(size = 0.1) +
...
read_yara(device_id_tur,from_date = "2023-10-01 00:00",
          until_date = as.character(lubridate::now()+days(1)),
          dest_file = dest)

data_new <- read_delim(dest,delim = '\t')

data_turgor <- data_new |> 
