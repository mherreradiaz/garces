...
files <- 'data/processed/espacial/raster/potencial_predict/' |> 
  list.files(full.names=T)

sitios <- str_extract(files,"(?<=xgb_)[a-zA-Z_]+(?=_\\d{4}_\\d{2}_\\d{2})")
...
write_rds(data,'data/processed/potencial_xgb_predict.rds')
...
data_plot <- data |> 
  ...
  pivot_longer(cols=c('potencial','t_media','rh_media','vpd_medio','eto','pp'),
               names_to='variable',
               values_to='valor') |> 
...
data_plot |> 
  ...
  filter(variable %in% c('potencial')) |> 
