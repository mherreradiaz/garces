...
data_turgor <- read_rds('data/data_processed/turgor_mediahora.rds')
...
data_pca |> 
  ...
  select(-fecha_hora) |> 
  distinct(sitio,temporada,fecha,hora,.keep_all = T) |> 
  write_rds('data/data_processed/clima_pca.rds')
...
write_rds(data_turgor_modelado,'data/data_processed/turgor_procesado_mediahora.rds')
...
data_turgor_modelado |> 
  ...
  write_rds('data/data_processed/turgor_procesado.rds')
