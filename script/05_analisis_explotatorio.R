...
(data_fluo |> # BOXPLOT 
  ...
  theme_light()) %>%
  ggsave('reporte/png/fluorescencia_boxplot.png', ., width = 7, height = 5, units = "in")
...
(data_fluo |> # SERIE DE TIEMPO
  ...
  theme_bw())  %>%
  ggsave('reporte/png/fluorescencia_serie.png', ., width = 9, height = 5, units = "in")
...
(data_potencial |> # BOXPLOT 
  ...
  theme_light())  %>%
  ggsave('reporte/png/potencial_boxplot.png', ., width = 7, height = 5, units = "in")
...
(data_potencial |> # SERIE DE TIEMPO
  ...
  facet_grid(tratamiento~sitio) +
  theme_bw()) %>%
  ggsave('reporte/png/potencial_serie.png', ., width = 9, height = 5, units = "in")
...
(data_cepto |> # BOXPLOT
  ...
  facet_grid(~sitio) +
  theme_light()) %>%
  ggsave('reporte/png/par_boxplot.png', ., width = 7, height = 5, units = "in")
...
(data_cepto |> # SERIE DE TIEMPO
  ...
  facet_grid(~sitio) +
  theme_bw()) %>%
  ggsave('reporte/png/par_serie.png', ., width = 9, height = 5, units = "in")
...
(data_sm |> # BOXPLOT
  ...
  theme_light()) %>%
  ggsave('reporte/png/zim_sm_boxplot.png', ., width = 7, height = 5, units = "in")
...
(data_sm |> # SERIE DE TIEMPO
  ...
  ggsave('reporte/png/zim_sm_serie.png', ., width = 9, height = 5, units = "in")
...
(data_turgor |> # BOXPLOT
   ...
   theme_light()) %>%
  ggsave('reporte/png/zim_turgor_boxplot.png', ., width = 7, height = 5, units = "in")
...
(data_turgor |> # SERIE DE TIEMPO ESTANDARIZADA
    ...
    ggplot(aes(fecha,value_sc,color=zim)) +
    geom_point(size=.05) +
    labs(y = 'Presión de parche') +
    facet_grid(tratamiento+sitio~unidad) +
    theme_bw() +
    guides(color = guide_legend(override.aes = list(size = 2)))) %>%
  ggsave('reporte/png/zim_turgor_serie_std.png', ., width = 9, height = 5, units = "in")
...
(data_turgor |> # SERIE DE TIEMPO
    ...
    mutate(unidad = factor(unidad, levels = 1:3)) |> 
    group_by(sitio,codigo,zim) |> 
    ggplot(aes(fecha,value,color=zim)) +
    geom_point(size=.05) +
    labs(y = 'Presión de parche') +
    facet_grid(tratamiento+sitio~unidad) +
    theme_bw() +
    guides(color = guide_legend(override.aes = list(size = 2)))) %>%
  ggsave('reporte/png/zim_turgor_serie.png', ., width = 9, height = 5, units = "in")
...
(data_turgor |> 
  ...
  mutate(unidad = as.factor(unidad),
         hora_dia = as.numeric(format(fecha,"%H"))) |> 
  ...
  theme_bw()) %>%
  ggsave('reporte/png/zim_turgor_ciclo.png', ., width = 9, height = 5, units = "in")
