...
files <- dir_ls('data/processed/espacial/raster/predictores')

dates <- str_extract(files,'[0-9]{4}_[0-9]{2}_[0-9]{2}') |> ymd()
...
out <- map(files,\(file){
  print(file)
  predictores <- rast(file)
  
  if(!(is.na(values(s2)) |> all())){
    ...
    names(predictores) <- tolower(names(predictores))
      
    sitio_filt <- str_extract(file,'(?<=SMOOTH_)[a-zA-Z_]+(?=_\\d{4}\\d{2}\\d{2})')
    fecha_filt <- gsub('_','-',str_extract(file,'\\d{4}\\d{2}\\d{2}'))
    data_clima_x <- data_clima |> 
      filter(sitio == sitio_filt,
             fecha  == ymd(fecha_filt))
    
    #crear raster para prediccion
    
    rast_temp <- predictores[[1]]
    values(rast_temp) <- data_clima_x$t_media
    rast_rh <- predictores[[1]]
    values(rast_rh) <- data_clima_x$rh_media
    rast_vpd <- predictores[[1]]
    values(rast_vpd) <- data_clima_x$vpd_medio
    rast_eto <- predictores[[1]]
    values(rast_eto) <- data_clima_x$eto
    rast_pp <- predictores[[1]]
    ...
    predictores <- c(rast_temp,rast_rh,rast_vpd,rast_eto,rast_pp,predictores)
    names(predictores)[1:5] <- c('t_media','rh_media','vpd_medio','eto','pp')    
    ...
    potencial_raster <- predictores[[1]]
