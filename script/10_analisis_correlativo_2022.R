...
data_fluo <- read_rds('data/data_processed_old/rds/data_fluorescencia.rds') |>
  mutate(tratamiento = substr(codigo,1,2),
         codigo = case_when(grepl("^T0", codigo) & sitio == "la_esperanza" ~ gsub("T0", "T0H", codigo),
                            TRUE ~ codigo)) |>
  select(sitio, fecha, codigo, tratamiento, 'Fv/Fm')

data_potencial <- read_csv('data/data_processed_old/csv/data_potencial.csv') |>
  mutate(tratamiento = substr(codigo,1,2)) |>
  select(sitio, fecha, codigo, tratamiento, potencial_bar)
...
data_cepto <- read.csv('data/data_processed_old/csv/data_ceptometro.csv', sep = ';') |>
  ...
  select(sitio, fecha, tratamiento, above_par)
...
data_sm <- read_rds('data/data_processed_old/rds/data_zim_sm.rds') |>
  ...
  mutate(codigo = paste0(tratamiento,codigo),
         VWC = value,
         fecha = date(hora)) |>
  select(sitio, fecha, codigo, tratamiento, VWC)

data_turgor <- read_rds('data/data_processed_old/rds/data_zim_turgor.rds') |>
  filter(hour(hora) %in% 13:14) |>
  ungroup() |>
  mutate(codigo = substr(paste0(tratamiento,codigo),1,str_length(codigo)),
         fecha = date(hora)) |>
  group_by(fecha,sitio,tratamiento, codigo) |>
  summarise(parche = mean(value,na.rm=T))

# matriz de correlación
data_join <- data_sm |> 
  left_join(data_turgor, by = c('sitio','fecha','codigo','tratamiento')) |>
  left_join(data_fluo, by = c('sitio','fecha','codigo','tratamiento')) |>
  left_join(data_potencial, by = c('sitio','fecha','codigo','tratamiento')) |>
  left_join(data_cepto, by = c('sitio','fecha','tratamiento'))

for (i in c('la_esperanza','rio_claro')) {
  matriz_cor <- data_join |>
    filter(sitio == i) |>
    select(VWC:above_par) |> 
  ...
  (ggplot(data = reshape2::melt(matriz_cor), aes(x = Var1, y = Var2, fill = value)) +
      ...
      labs(x = "", y = "")) %>%
    ggsave(paste0('reporte/png_2022/matriz_cor_',i,'.png'), ., width = 7, height = 6, units = "in")
}
