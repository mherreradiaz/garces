...
for (x in 1:length(files_le)) {
  ...
  r_le_index <- c(ndwi_le,ndmi_le,msi_le,gci_le,ndvi_le,nbr_le,ndmi_le,dwsi_le,ndvi_705_le,b_i_le)
  names(r_le_index) <- c('ndwi','ndmi','msi','gci','ndvi','nbr','nmdi','dwsi','ndvi_705','b_i')
  
  r_rc_index <- c(ndwi_rc,ndmi_rc,msi_rc,gci_rc,ndvi_rc,nbr_rc,ndmi_rc,dwsi_rc,ndvi_705_rc, b_i_rc)
  names(r_rc_index) <- c('ndwi','ndmi','msi','gci','ndvi','nbr','nmdi','dwsi','ndvi_705','b_i')
