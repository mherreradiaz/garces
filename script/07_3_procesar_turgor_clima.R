...
dest <- paste0('data/raw/turgor/turgor_',
...
write_rds(data_clima,paste0('data/raw/clima/clima_',año,'_',sprintf('%02d',mes),'.rds'))

# procesar y combinar datos brutos de turgor y clima

files_turgor <- list.files('data/raw/turgor/',full.names = T)
...
data_turgor <- bind_rows(data_turgor_new) |>
  ...
  write_rds('data/processed/turgor_mediahora.rds')
...
data_clima <- data_clima |> 
  mutate(fecha = fecha_f(datetime),
         ...
         .before = datetime) |> 
  select(-datetime) |> 
  write_rds('data/processed/clima_mediahora.rds')