...
filtro <- data_turgor |>
  ...
  drop_na() |>
  mutate(turgor_sc = scale(turgor)) |>
  group_by(temporada,sensor,cluster,hora) |>
  summarize(turgor_hora = mean(turgor_sc, na.rm = TRUE)) |>
  group_by(temporada,sensor,cluster) |>
...
data_turgor <- data_turgor |>
  left_join(filtro, by = c('temporada','sensor','cluster')) |>
  filter(filtro == 1) |>
  select(-filtro)

{
  ...
  for (x in 1:nrow(codigos)) {
    ...
    png(paste0('reporte/analisis_turgor/plot_limpiado/', gsub('-','_',str_replace_all(plot_name, "\\s", "_")), '.png'),
        width = 14 * 300, height = 7 * 300, units = "px", res = 300)
    print(plot_final)
    dev.off()
