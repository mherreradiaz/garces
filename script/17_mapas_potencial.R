...
ids <- match(sort(dates_max_cov_rc),names(swp_rc))

swp_rc_mxcov <- subset(swp_rc,ids)

map1 <- tm_shape(swp_rc_mxcov*0.1) +
  tm_raster(title = expression(Psi[S]~~(MPa)),style = 'cont', palette = diverging_hcl(palette = 'Green-Brown',n=7) ) +
  tm_facets(nrow = 2,free.scales = TRUE) +
  tm_layout(legend.outside = TRUE,
            panel.label.bg.color = 'white',
            panel.labels = ymd(names(swp_rc_mxcov)))
...
ids <- match(sort(dates_max_cov_le),names(swp_le))
