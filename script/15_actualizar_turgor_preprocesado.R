...
data_old <- read_rds('turgor_preproceso/data_processed/zim_turgor.rds')
from_date <- data_old$fecha |> max() |> substr(1,16)

dest <- paste0('turgor_preproceso/data_raw/zim_turgor',
...
write_rds(data_turgor,'turgor_preproceso/data_processed/zim_turgor.rds')

# Clima

data_old <- read_rds('turgor_preproceso/data_processed/clima.rds')
...
write_rds(data_clima, 'turgor_preproceso/data_processed/clima.rds')

data <- data_clima |> 
  mutate(datetime = as.POSIXct(paste(fecha,hora),format='%Y-%m-%d %H:%M'),
         datetime_03 = as_datetime(datetime,tz = 'America/Santiago'))


for (año in 2022:2023) {
  
  for (mes in sprintf("%02d", c(9:12,1:4))) {
    
    if (mes == '01') {año <- año +1}
    
    periodo <- dia(año,as.numeric(mes))
    ...
    write_rds(data_new,paste0('turgor_preproceso/data_raw/clima_',año,'_',mes,'.rds'))
  }
}

files <- grep('clima',list.files('turgor_preproceso/data_raw',full.names = T),value=T)

lista <- list()

for (x in 1:length(files)) {lista[[x]] <- read_rds(files[x])}

data_clima <- bind_rows(lista)
  
data_clima <- data_clima |> 
  filter(sitio == 'la_esperanza',
         between(fecha,'2023-02-08','2023-02-10')) |> 
  view()

# Preprocesado

data_turgor <- read_rds('turgor_preproceso/data_processed/zim_turgor.rds')
data_clima <- read_rds('turgor_preproceso/data_processed/clima.rds')

data_cor <- data_turgor |> 
  left_join(data_clima, by = c('sitio','fecha','hora')) |> 
...
data_t <- data_cor |> 
  distinct(sitio,temporada,fecha,hora,t_sc) |> 
...
data_vpd <- data_cor |> 
  distinct(sitio,temporada,fecha,hora,vpd_sc) |> 
  left_join(data_ccf,by=c('sitio','temporada')) |> 
...
data_as <- data_cor |>
  left_join(data_ccf,by=c('sitio','temporada')) |> 
  mutate(datetime = as.POSIXct(paste(fecha,hora),format = '%Y-%m-%d %H:%M')-(30*60)*t_lag,
         fecha = as.POSIXct(datetime,format = '%Y-%m-$d'),
         hora = as.POSIXct(datetime,format = '%H:%M'))
  ...
  data_cor |> 
    filter(sitio == 'la_esperanza',temporada == '2022-2023',sensor == 8569) |> 
    mutate(NOSE = lead(t_sc,3)) |> 
    view()
